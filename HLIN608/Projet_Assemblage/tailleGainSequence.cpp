...
int main(int argc, char *argv[]){
	if(argc!=3){
		cout<<"Il faut trois arguments"<<endl;
	}
	else{
		string L0 = "";
		string L1 = "";
		...
		for(int n=0; n<nb; n++){
			...
			L0.append(to_string(motFusion0.size()));
			L0.append("\\\\");
			...
			L1.append(to_string(motFusion1.size()));
			L1.append("\\\\");

			G1.append(to_string((t*10)-motFusion1.size()));
			G1.append("\\\\");

			GP1.append(to_string(100-((motFusion1.size()/(t*10.))*100.)));
			GP1.append("\\\\");
		...
		// Taille superséquence
		string const nomFichier("tailleSupersequence/TailleSequence_lg"+to_string(t)+"_nb"+to_string(nb)+".txt");
    	ofstream monFlux(nomFichier.c_str());

	    if(monFlux){
	    	monFlux << "Calcul de la taille de "+to_string(nb)+" superséquences à partir de 10 séquences de taille "+to_string(t) << endl << endl;
	        
	        monFlux << "NAIF" << endl;
	        monFlux<<L0<<endl<<endl;
	        
	        monFlux << "OPTI 1" << endl;
	        monFlux<<L1<<endl<<endl;
	    ...
	    // Gain superséquence
	    string const nomFichier2("gainSupersequence/GainSequence_lg"+to_string(t)+"_nb"+to_string(nb)+".txt");
    	ofstream monFlux2(nomFichier2.c_str());

	    if(monFlux2){
	    	monFlux2 << "Calcul du gain de nucléotides sur "+to_string(nb)+" superséquences à partir de 10 séquences de taille "+to_string(t) << endl << endl;
	        
	        monFlux2 << "NAIF" << endl;
	        monFlux2<<G0<<endl<<endl;
	        
	        monFlux2 << "OPTI 1" << endl;
	        monFlux2<<G1<<endl<<endl;
	    ...
	    string const nomFichier3("gainSupersequence/GainPourcentageSequence_lg"+to_string(t)+"_nb"+to_string(nb)+".txt");
    	ofstream monFlux3(nomFichier3.c_str());

	    if(monFlux3){
	    	monFlux3 << "Calcul du gain de pourcentage de nucléotides sur "+to_string(nb)+" superséquences à partir de 10 séquences de taille "+to_string(t) << endl << endl;
	        
	        monFlux3 << "NAIF" << endl;
	        monFlux3<<GP0<<endl<<endl;
	        
	        monFlux3 << "OPTI 1" << endl;
	        monFlux3<<GP1<<endl<<endl;
