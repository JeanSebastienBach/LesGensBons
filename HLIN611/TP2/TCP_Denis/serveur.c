...
int recvTCP(int socket, char *buffer, size_t length, unsigned int *nbBytesReceved, unsigned int * nbCallRecv){
	int nbOctetsRecu = 0;
  int received = 0;
    while (nbOctetsRecu < length){
  
      received = recv(socket,buffer + nbOctetsRecu,length - nbOctetsRecu,0);

      if(received < 0){
        perror("Erreur reveived");
        return -1;
      }
      else if(received == 0) {
        printf("problème avec received");
        return 0;
      }
      else {
      nbOctetsRecu += received;
