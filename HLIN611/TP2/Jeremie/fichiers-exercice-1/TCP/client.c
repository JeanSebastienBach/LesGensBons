...
int sendTCP(int socket, const char * buffer, size_t length, unsigned int *nbBytesSent, unsigned int * nbCallSend){
    // (*nbBytesSent) : Nombre d'octets envoyés au total
    // (*nbCallSend) :  Nombre de fois où la fonction send est appelée
    int sent; // Nombre d'octets envoyés lors d'un appel à send
    int nbTotalSent = 0; // Nombre d'octets envoyés depuis le début de cet appel à sendTCP
