...
int recvTCP(int socket, char *buffer, size_t length, unsigned int *nbBytesRecv unsigned int * nbCallRecv){
	// (*nbBytesRecv) : Nombre d'octets reçus au total
    // (*nbCallRecv) :  Nombre de fois où la fonction call est appelée
    int recv; // Nombre d'octets reçus lors d'un appel à send
    int nbTotalRecv = 0; // Nombre d'octets reçus depuis le début de cet appel à recvTCP
    while (nbTotalRecv < length){
    	// Si le client n'a pas reçu le bon nombre d'octets, il boucle
      	recv = recv(socket, buffer + nbTotalRecv, length - nbTotalRecv, 0);
