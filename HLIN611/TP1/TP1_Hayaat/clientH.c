...
int main(int argc, char *argv[]) {
  ...
  m[strlen(m)-1]  = '\0'; // je retire le saut de ligne et le remplace par le symbole de fin de chaine                                    
  ...
  int snd = send(ds, m , strlen(m)+1,0);  //on rajoute 1 pour transmetrre le symbole de fin de chaine /0
  ...
  if (snd < sizeof(int))
  {
      printf("Client : attention, je n'ai reçu qu'une partie du message attendu; je dois réceptionner le reste\n");
      close (ds);
      exit (1);
  } 
  ...
  if (rcv<sizeof(int))
  {
    printf("Client : attention je n'ai pu deposer qu'une partie du message, je dois attendre le reste\n");
    close(ds);
    exit(1);
    //pour ce premier exercice , ce cas ne se produira.
  }
