...
int main(int argc, char *argv[])
{
  ...
  int ds = socket(PF-INET, SOCK_STREAM, 0);
...
*/
  ...
  if(bind(ds, (struct sockaddr *) &serverAddr, sizeof(serrverAddr)) < 0){
  ...
  // socket serveur pour attendre les clients (bind, listen, accept)
  // socket client (du côté client) qui sera utilisé pour (connect)
  // le accept (côté serveur) me créé un nouveau socket côté servuer 
  // pour dialoguer avec ce client : dsCv
  // Car on créé un circuit virtuel en mode connecté (dscv, socketclient connu par sa sockaddr_in)
  ...
  // char * inet_ntoa(struct in_addr in) or on a l'adresse client :
  // struct sock sockaddr_in adCv
  ...
  // Je suis bloqué tant que y'a pas quelque chose qui arrive
  ...
  if(rcv<0){
  	...
  	close(dsCv);
  	// Se poser la question de continuer ou pas l'exécution
  }
